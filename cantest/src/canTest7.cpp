...
void calLinkSysXYFromAngle(LinkSys linksys,float angle1,float angle2)
{
	...
	linksys.x1 = linksys.L1*cos(linksys.angle1)+linksys.L2*cos(linksys.angle2);
	linksys.y1 = linksys.L1*sin(linksys.angle1)+linksys.L2*sin(linksys.angle2);
}

void calLinkSysangleFromXY_1(LinkSys linksys,float x,float y)
{
	//TODO: 
	linksys.angle01 = atan(linksys.y1/linksys.x1);
	linksys.L0 = sqrt(pow(linksys.x1,2)+pow(linksys.y1,2));

	float angle11 = acos((pow(linksys.x1,2)+pow(linksys.y1,2)+pow(linksys.L1,2)-pow(linksys.L2,2))/(2*linksys.L0*linksys.L1))+linksys.angle01;
	float angle12 = -acos((pow(linksys.x1,2)+pow(linksys.y1,2)+pow(linksys.L1,2)-pow(linksys.L2,2))/(2*linksys.L0*linksys.L1))+linksys.angle01;
	float angle21 = -acos((pow(linksys.x1,2)+pow(linksys.y1,2)+pow(linksys.L2,2)-pow(linksys.L1,2))/(2*linksys.L0*linksys.L2))+linksys.angle01;
	float angle22 = acos((pow(linksys.x1,2)+pow(linksys.y1,2)+pow(linksys.L2,2)-pow(linksys.L1,2))/(2*linksys.L0*linksys.L2))+linksys.angle01;
...
void targetAngleSub2_Callback(std_msgs::Float32MultiArray targetAngleMessage)
{
	linksys.angle1 = targetAngleMessage.data[0];
	linksys.angle2 = targetAngleMessage.data[1];
	linksys.angle3 = targetAngleMessage.data[2];
	linksys.angle4 = targetAngleMessage.data[3];

	motor[0].targetPosition = motor[0].positionIni - linksys.z2/linksys.z1*(linksys.angle1 - linksys.angle1Ini)*8192/(2*M_PI);
	motor[1].targetPosition = motor[1].positionIni + linksys.z2/linksys.z1*(linksys.angle2 - linksys.angle2Ini)*8192/(2*M_PI);

	motor[2].targetPosition = motor[2].positionIni - linksys.z4/linksys.z3*(linksys.angle3 - linksys.angle3Ini)*8192/(2*M_PI);
	motor[3].targetPosition = motor[3].positionIni + linksys.z4/linksys.z3*(linksys.angle4 - linksys.angle4Ini)*8192/(2*M_PI);
}
    ...
    for (i = 0;; i++)
    {
		...
		if(i%4==0)
		{
			switch (mode)
			{
				...
				case 7:
				// TODO 完成两个电机在笛卡尔坐标系下运动即，给定笛卡尔坐标系下的位置，然后得到两个电机的角度值，并利用位置控制模式使末端执行器固定在那个位置上
					printf("mode is %d; ",mode);
                ...
                case 8:
                // TODO 完成两个单边在笛卡尔坐标系下面的运动
					printf("mode is %d; ",mode);
					printf("l1 a1 a2 a3 a4 are: %f %f %f %f;",linksys.angle1,linksys.angle2,linksys.angle3,linksys.angle4);
					printf("a1_ini a2_ini a3_ini a4_ini are :%f %f %f %f;",linksys.angle1Ini,linksys.angle2Ini,linksys.angle3Ini,linksys.angle4Ini);
					printf("pos_ini are :%d %d %d %d;",motor[0].positionIni,motor[1].positionIni,motor[2].positionIni,motor[3].positionIni);
		...
		if(rxCounter == 32&& ((mode == 5 || mode == 6 || mode == 7 || mode == 8)))
		{
			flag = 1;
			for (j = 0;j<4;j++)
			{
